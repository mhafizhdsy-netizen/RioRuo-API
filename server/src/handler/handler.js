...
const singleMovieHandler = async (req, res) => {
    const { year, month, slug: movieTitleSlug } = req.params;
    const fullSlugForMovieUtil = `/${year}/${month}/${movieTitleSlug}`;
    console.log('fullSlugForMovieUtil: ', fullSlugForMovieUtil);
    let data;
    try {
        data = await otakudesu.movie(fullSlugForMovieUtil);
    }
    catch (e) {
